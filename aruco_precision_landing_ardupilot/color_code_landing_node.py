...
class LandingPublisher(Node):
    ...
    def __init__(self, username=None):
        ...
        #         print('INVALID USER')
        # else:
        #     self.cam_sub = self.create_subscription(Image, '/uav1/camera_down', self.aruco_callback, 10)
...
# ange.lower_bound.ch1 = 105;
# range.lower_bound.ch2 = 68;
# range.lower_bound.ch3 = 149;


# range.upper_bound.ch1 = 174;
# range.upper_bound.ch2 = 113;
# range.upper_bound.ch3 = 255;

            low_color = np.array([105, 68, 149])
            high_color = np.array([174, 113, 255])
            color_mask = cv2.inRange(hsv, low_color, high_color)
            
            kernel = np.ones((5, 5), np.uint8)


            color_mask = cv2.erode(color_mask, kernel, iterations=3) 
            color_mask = cv2.dilate(color_mask, kernel, iterations=3) 

            contours, hierarchies = cv2.findContours(color_mask, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)

            if len(contours) > 0:
                contour = max(contours, key = cv2.contourArea)
                
                cv2.drawContours(image_detected, [contour], 0, (0,255,0), 3)
